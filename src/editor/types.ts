...
export type ClipType = MediaKind | "text" | "shape";
export type TrackType = "video" | "audio" | "text" | "shape";
...
export function isTextual(clip: Clip): boolean { return clip.type === "text"; }
export function hasVisual(clip: Clip): boolean { return clip.type !== "audio"; }
export function trackTypeFor(clipType: ClipType): TrackType { if (clipType === "audio") return "audio"; if (clipType === "text") return "text"; if (clipType === "shape") return "shape"; return "video"; }
export function normalizeProject(project: Project): Project {
  const tracks = project.tracks.map((track) => ({ ...track, clips: track.clips.map((clip) => ({ ...clip, keyframes: clip.keyframes ?? [], audio: { ...DEFAULT_AUDIO, ...(clip.audio ?? {}) }, transform: { ...DEFAULT_TRANSFORM, ...clip.transform }, effects: clip.effects ?? [], ...(clip.type === "text" ? { text: clip.text ?? "Text", style: { ...DEFAULT_TEXT_STYLE, ...(clip.style ?? {}) } } : {}), ...(clip.type === "shape" ? { shape: { kind: "rectangle" as ShapeKind, fill: "#ffffff", fillOpacity: 100, stroke: "#000000", strokeWidth: 0, radius: 0, ...(clip.shape ?? {}) } } : {}) })) }));
  const ensure = (id: string, name: string, type: TrackType) => tracks.some((t) => t.type === type) ? null : { id, name, type, clips: [] };
  const extra = [ensure("V1", "VIDEO", "video"), ensure("A1", "AUDIO", "audio"), ensure("T1", "TEXT", "text"), ensure("S1", "SHAPES", "shape")].filter((t): t is Track => t !== null);
